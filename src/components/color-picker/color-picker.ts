...
export class ColorPickerComponent {
  ...
  ionViewDidLoad() {
    // Si une couleur a été définie via l'attribut de la directive
    if(this.hexColor){
    ...
    // Si une couleur a été définie via la modal
    let colorParam = this.navParams.get('colorParam');
    if(colorParam) {
      this.colorFromChooser = colorParam;
    } else{
      this.colorFromChooser = "#0000FF";
    }
    this.init();
  ...
  initPalette(){
    ...
    // let colorFromChooserRgb = this.toRgb(this.colorFromChooser);
    // console.log(this.colorFromChooser);
    // console.log(colorFromChooserRgb);
    // let paletteData = this.ctxPalette.getImageData(0, 0, canvasPalette.width, canvasPalette.height).data;
    // let pixel = 0;
    // let selectorX = this.ctxPalette.canvas.width / 2;
    // let selectorY = this.ctxPalette.canvas.height / 2;
    // for (var i = 0; i < paletteData.length; i += 4) {
    //   if(paletteData[i] == colorFromChooserRgb.r && paletteData[i+1] == colorFromChooserRgb.g && paletteData[i+2] == colorFromChooserRgb.b) {
    //     console.log('OK');
    //     // On test bien qu'on est pas au premier pixel
    //     if (i>0) {
    //       pixel = i/4;
    //     }
        
    //     selectorX = pixel%this.ctxPalette.canvas.width;
    //     selectorY = (pixel-selectorX)/this.ctxPalette.canvas.width;
    //   }
    // }

    // this.drawSelector(this.ctxPalette, selectorX, selectorY);
  ...
  // Méthode de validation de la modale
  sumbitColor() {
    // Ferme la modal en renvoyant les parametres du slot à créer
